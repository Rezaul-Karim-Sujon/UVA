...
ll bigmod(ll n,ll p,ll MOD){if(p==0) return 1;if(p==1)return (n+MOD)%MOD;if(p%2)return (bigmod(n,p-1,MOD)*n+MOD)%MOD;else{ll x=bigmod(n,p/2,MOD);return (x*x+MOD)%MOD;}}
